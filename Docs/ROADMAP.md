...
#### Phase 4.1 — 模型无关的音频原生 Live 会话（待迭代）

目标不是给现有 ASR→文本模型→TTS 管线换一个名称，而是增加一条可持续双工、可插话、可调用工具的实时会话通道：

- 定义 `RealtimeConversationTransport` 中立协议，通过能力协商接入支持实时音频的模型；不得写死某个模型或供应商名单。
- 支持 WebRTC / WebSocket 长连接、音频与文字增量、服务端断句、发声中持续收音、插话取消和断线续接。
- 模型发起的工具调用继续进入灵枢现有工具总线、子线程和任务账本；子线程完成结果必须回注当前 Live 上下文与主线程记忆。
- 图片和屏幕沿用现有感知链，以关键帧/事件方式注入实时会话，不把持续视频误当作普通文本附件。
- 首次按能力尝试原生 Live；明确不支持或连接失败后，降级到现有 ASR→大脑→流式 TTS，并缓存该模型的能力结论供后续复用。

**验收**：用户可在灵枢发声期间自然插话；取消后旧音频不再续播；工具执行前后保持同一对话上下文；实时通道不可用时自动降级且不丢当前轮输入。
