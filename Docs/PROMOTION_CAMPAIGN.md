...
- The first task now ends in a locale-specific structured Alpha first-run report ([English](https://github.com/RoyZhao1991/LingShu/issues/new?template=first_run_report.yml) / [中文](https://github.com/RoyZhao1991/LingShu/issues/new?template=first_run_report_zh.yml)), including success, partial success, and failure instead of counting downloads as installations. Environment details and long narratives are optional so a successful run can be reported with the comparable core fields only.
...
| 2026-07-19 | Developers Digest project tip | Attempted one tailored, non-sponsored English project tip, but the recipient address omitted the final `s` in `developers`. Gmail returned a permanent delivery failure (`19f77bf6524f8abb`), so this attempt did not reach the publication and must not be counted as delivered outreach | Preserve the failed attempt as an audit record; correct the address only after verifying the publication's current official contact page, and do not infer editorial review or reach from the original sent-mail entry |
...
| 2026-07-19 | Signed release, Homebrew, website, and canonical announcement | Published [`v0.1.0-alpha.7`](https://github.com/RoyZhao1991/LingShu/releases/tag/v0.1.0-alpha.7) Build 10 from clean source `36fa7ce`, updated the public Cask and website, and added one factual [release update](https://github.com/RoyZhao1991/LingShu/discussions/12#discussioncomment-17687162) to the existing launch thread. The app and DMG passed separate Apple notarization, stapling, remount verification, and Homebrew SHA-256 fetch validation | Measure attributable visits, release requests, first-run reports, Issues, and outside PRs for seven full days. The first request for each new release asset was maintainer verification and must be excluded from outside conversion claims |
| 2026-07-19 | `awesome-ai-coding-tools` Coding Agents | Submitted one category-specific factual listing through [PR #553](https://github.com/ai-for-developers/awesome-ai-coding-tools/pull/553). The entry describes LingShu as an Apache-2.0 native macOS execution agent with a replaceable model backend and an open runtime for code, documents, presentations, and authorized computer actions; the pull request discloses macOS 14+ and Alpha limitations | Track review and merge status, attributable referrers, release requests above the maintainer-verification baseline, first-run reports, Issues, and outside pull requests for seven days. Do not count an open pull request as a directory placement |
| 2026-07-19 | LocalAlternative local-first AI directory | Submitted LingShu through the directory's public review form with repository, website, model-agnostic local-runtime description, signed Alpha, Homebrew path, and current limitations. The submission endpoint returned review id `jx76g6ddq9fvsw2a6k91h7f2h18ath4s` | Treat this as confirmed delivery to a review queue only. Track publication, attributable referrer, release requests, first-run reports, Issues, and outside contributions for seven days; do not resubmit while review is pending |
| 2026-07-19 | README, website, and first-run feedback funnel | Split the privacy-safe first-run report into locale-matched [English](https://github.com/RoyZhao1991/LingShu/issues/new?template=first_run_report.yml) and [Chinese](https://github.com/RoyZhao1991/LingShu/issues/new?template=first_run_report_zh.yml) forms with identical core outcome fields, while making environment details and long narratives optional. Verified the language-linked route and layout at desktop and mobile widths | This improves first-value reporting rather than distribution. Measure explicit successful, partial, and failed reports and recurring setup blockers for seven days; do not infer use from downloads, clones, or Stars |
| 2026-07-19 | Native app first-run feedback path | Added localized, passive links for the first-run report, GitHub Discussions, and source repository to the standard macOS Help menu and Settings → Self-check. The report URL follows the configured application language, and dedicated tests cover both templates and canonical repository paths | Measure completed structured reports and substantive community questions for seven days. Do not count menu visibility or link opens as successful use, and do not add a nagging completion popup without evidence that it is needed |
| 2026-07-19 | Contributor onboarding and issue hygiene | Published the external pull-request review and recognition contract in both languages in `CONTRIBUTING.md`: review scope includes behavior, regressions, security, privacy, internationalization, documentation, and risk-proportional tests; merge requires sufficient evidence and passing CI; merged work keeps Git authorship and receives a specific public thank-you plus a release or operations record. Closed [Issue #2](https://github.com/RoyZhao1991/LingShu/issues/2) only after verifying every documented acceptance criterion on current `main` | Use this contract for the first outside Draft PR. Track review time, requested revisions, CI evidence, authorship preservation, and the exact post-merge recognition; do not count contributor intent as a merged contribution |
| 2026-07-19 | Signed release, clean-user gate, Homebrew, and Issue #7 | Published [`v0.1.0-alpha.8`](https://github.com/RoyZhao1991/LingShu/releases/tag/v0.1.0-alpha.8) Build 11 from exact green source `081976b`. The Apple-notarized DMG passed a disposable-user installation with 11/11 machine-readable checks covering real first-launch setup views, isolated preferences and Application Support, disabled Keychain and permission services, empty task history, app liveness, installed signature, and a deterministic local reply. Updated the public Cask to `0.1.0-11`, verified the remote SHA-256 fetch, published the result asset, and closed [Issue #7](https://github.com/RoyZhao1991/LingShu/issues/7) with evidence | Treat the first DMG request as maintainer Homebrew verification, not outside adoption. Measure requests above that baseline, privacy-safe first-run reports, reproducible Issues, and outside pull requests for seven days |
| 2026-07-20 | MacStories editorial tip | Sent one tailored, non-sponsored project tip to MacStories managing editor John Voorhees through the publication's [public contact route](https://www.macstories.net/about/). The note connects LingShu to Mac-native apps, automation, and agentic coding coverage; links the repository, 71-second demo, architecture map, signed Alpha 8 release, and Homebrew command; and explicitly discloses macOS 14+, BYOK, Alpha, and human-review limits | Treat Gmail message `19f7b25325fca7eb` as delivery evidence only. Do not follow up inside the normal editorial window. Monitor replies, attributable referrers, release requests above maintainer verification, first-run reports, Issues, and outside pull requests for seven days |
| 2026-07-20 | Website, README, and first-run report | Replaced the misleading three-minute quick-start label with a first-15-minutes path, stated the BYOK/token prerequisite before installation, exposed the three visible success signals, linked a no-token visitor to the real 71-second sample, and made success, partial success, and failure equally easy to report. Updated both report forms to the current Alpha 8 placeholder. The built site passed all 55 local resource references plus English and Chinese desktop/mobile checks with zero horizontal overflow | This repairs first-run conversion rather than adding reach. For seven full days, count only completed privacy-safe reports and recurring setup blockers; do not infer successful use from visits, clones, release requests, or Stars |
| 2026-07-20 | Developers Digest delivery correction | Verified the publication's current [official contact page](https://www.developersdigest.tech/contact), corrected the failed singular-address attempt to the published plural address, and sent one updated, audience-specific project tip. The message links the current Alpha 8 release and 71-second proof, and discloses macOS 14+, BYOK, no included inference credits, Alpha status, and human-approval limits | Gmail message `19f7e56157c63f39` proves that the corrected message entered Sent mail only; it is not evidence of inbox placement, editorial review, publication, traffic, or adoption. This restores the intended July 19 contact rather than adding a new channel. Send no further proactive outreach today and do not follow up during the normal editorial window |

Daily open-source growth operations now check metrics, distribution gates, review feedback, outside pull requests, and one concrete channel action. Outside PRs take priority over routine promotion: review first, merge only after verification, then record and thank the contribution. PR #76 is closed as duplicate and needs no follow-up. Do not submit another general-purpose directory pull request or Issue until Issue #348 is reviewed or seven days have elapsed. PR #2355 is already merged and should only be monitored for attributable downstream signals. Console.dev, the corrected Developers Digest delivery, and Forkable are targeted editorial pitches rather than directory listings; measure them independently and do not count their potential audiences as achieved reach. E2B PR #1271 and `awesome-ai-coding-tools` PR #553 are distinct curated surfaces and remain pending. LocalAlternative submission `jx76g6ddq9fvsw2a6k91h7f2h18ath4s` is in editorial review. Community launches must not reuse directory wording verbatim.
