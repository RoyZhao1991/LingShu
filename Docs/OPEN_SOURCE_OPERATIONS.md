...
当前发布状态：仓库已经公开；`v0.1.0-alpha.9` Build 12 从提交 `84d62e1` 的干净源码生成。Grok 派生源码已作为灵枢进程内原生 Loop Runtime 随 Universal App 打包，Maker 与 checker 使用隔离运行时会话，显式配置的 Codex 等外部 Agent 覆盖保持不变。Universal DMG 已完成 Developer ID 签名、App 与 DMG 两次 Apple 公证、票据装订、Runtime 双架构与签名校验、包内源码版本与 SHA-256 登记，并从该公证 DMG 进入一次性用户目录完成真实安装启动。机器可读结果中的 11 项检查全部通过；Release 资产同时公开 DMG、校验文件、构建清单与全新用户验证结果。该验证不等于外部用户首跑或真实远程模型配置成功，更多外部反馈仍是最高优先级。

当前增长漏斗仍处于“有发现、无首跑证明”阶段：GitHub 已出现真实独立访客、外部 Star、Fork 和 Release 请求，但尚无隐私安全的外部首跑报告。15 分钟测试招募帖已经补充 71 秒演示和统一反馈入口；下一优先级是把访问转成可复现首跑或贡献，而不是继续堆叠同类目录提交。
...
- [x] 从公证 DMG 在一次性用户目录完成首次设置入口、空历史、无权限基线和最小本地直答验证，并公开机器可读结果。
