...
当前发布状态：仓库已经公开；`v0.1.0-alpha.8` Build 11 从提交 `081976b` 的干净源码生成。Universal DMG 已完成 Developer ID 签名、App 与 DMG 两次 Apple 公证、票据装订、包内源码版本与 SHA-256 登记，并从该公证 DMG 进入一次性用户目录完成真实安装启动。机器可读结果中的 11 项检查全部通过，覆盖语言选择、无主脑配置入口、空任务历史、偏好与 Application Support 隔离、钥匙串与权限服务禁用、App 存活、安装签名和最小本地直答；结果已作为 Release 资产公开。Homebrew Cask 已更新到 `0.1.0-11` 并完成公网拉取校验。该验证不等于外部用户首跑或真实远程模型配置成功，更多外部反馈仍是最高优先级。
