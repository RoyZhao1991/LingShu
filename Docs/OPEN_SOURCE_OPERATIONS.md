...
当前发布状态：仓库已经公开；`v0.1.0-alpha.7` 从提交 `36fa7ce` 的干净源码生成。Build 10 修复交付回复仅包含文件名时的产物漏登，同时加入用户消息隔离、可信目录边界、规范路径去重和符号链接逃逸防护；Universal DMG 已完成 Developer ID 签名、App 与 DMG 两次 Apple 公证、票据装订、包内源码版本校验和四类 SHA-256 登记。71.2 秒公开演示已经完成并与该安装包行为一致；干净 Mac 首跑与更多外部用户反馈继续作为高优先级增强，不虚构为已经完成。
