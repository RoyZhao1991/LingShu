...
- The app records the exact Git source revision in its bundle metadata. Build 6 identifies `9c5aeec7c77e23891acf8d3606023058ffffd05a`.
- `release-manifest.json` records the source archive, app binary, CLI binary, and DMG SHA-256 values together with both Apple notarization request IDs.
- The release script rejects a dirty source tree and verifies the bundled source revision before creating the DMG.

### Install

1. Download the Universal DMG and its `.sha256` file from this release.
2. Verify the download with `shasum -a 256 -c LingShu-0.1.0-6-macOS-universal.dmg.sha256`.
...
The optional CLI is bundled inside the installed app. See the [CLI and connector guide](https://github.com/RoyZhao1991/LingShu/blob/main/Docs/CLI.md) for setup, JSON output, exit codes, and the external-connector pattern.
...
Please report reproducible defects with the bug template. Security issues should be reported privately according to the [security policy](https://github.com/RoyZhao1991/LingShu/blob/main/SECURITY.md).
...
- App 在包元数据中记录精确的 Git 源码提交号；Build 6 对应 `9c5aeec7c77e23891acf8d3606023058ffffd05a`。
- `release-manifest.json` 同时记录源码归档、App 二进制、CLI 二进制与 DMG 的 SHA-256，以及两次 Apple 公证请求编号。
- 发布脚本拒绝从脏工作区构建，并在生成 DMG 前校验包内源码版本。

### 安装方式

1. 下载本 Release 中的 Universal DMG 与对应 `.sha256` 文件。
2. 运行 `shasum -a 256 -c LingShu-0.1.0-6-macOS-universal.dmg.sha256` 校验文件。
3. 打开 DMG，将 `灵枢.app` 拖入“应用程序”。
4. 启动灵枢，并按引导配置主脑通道。
5. 仅在所选能力确实需要时授予对应 macOS 权限。

可选 CLI 已随安装后的 App 一并交付。安装方式、JSON 返回、退出码与外部连接器范式见 [CLI 与连接器指南](https://github.com/RoyZhao1991/LingShu/blob/main/Docs/CLI.md)。
...
请使用 Bug 模板提交可复现问题；安全问题请按[安全策略](https://github.com/RoyZhao1991/LingShu/blob/main/SECURITY.md)通过私密渠道报告。
