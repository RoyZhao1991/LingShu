...
执行过程中若发现必须由人完成扫码、外部登录、实体操作、选文件、确认或其它人机协作，填 human_interaction 对象：{kind,title,prompt,payload,options,completion_probe,resume_token,source}。这不是失败；任何 planner、worker、checker 或工具都可以提出，流程会暂停当前节点并在完成后续跑。不要把它写进 OAuth，也不要只写在 reply 文本里。
承认缺能力、缺前提、部分完成、等待用户、需要人机交互或需要自我补齐能力，都必须通过 completion.status / completion.needs_user / user_input / human_interaction / inability / OAuth 表达，不允许只写在 reply 文本里。
"""
    ...
    var declaresUserBlock: Bool {
        ...
        || (humanInteraction?.normalized != nil)
    ...
    private static func parseObject(_ obj: [String: Any]) -> LingShuStructuredModelOutput? {
        let hasKnownKey = obj.keys.contains("reply")
            ...
            || obj.keys.contains("humanInteraction")
            || obj.keys.contains("human_interaction")
        ...
        let humanInteraction = LingShuHumanInteractionRequest.parse(obj["humanInteraction"] ?? obj["human_interaction"])
        let oauth = LingShuOAuthAuthorizationRequest.parse(obj["OAuth"] ?? obj["oauth"])
        guard !reply.isEmpty || completion != nil || userInput != nil || humanInteraction != nil || inability != nil || oauth != nil else { return nil }
        return .init(reply: reply, completion: completion, userInput: userInput, humanInteraction: humanInteraction, inability: inability, OAuth: oauth)
    }
