...
extension LingShuState {
    ...
    private func registerAgentTool() -> LingShuAgentTool {
        LingShuAgentTool(
            name: "register_agent",
            description: "把**本机一个外部 CLI agent** 注册成 agent 插件(被主人告知『本机有 X、怎么调』时用)。注册后即可 @它 声明式编排、或 run_agent 委托。入参:id(稳定标识如 codex)、name(显示名)、executable(可执行文件路径或命令名)、args(参数模板数组,用 {{objective}} 占位)、role(maker/checker/general)、aliases(可选别名)。**关键·权限:开发/产出类(maker)agent 的 args 必须带足够的写权限标志,否则它跑在只读沙箱里写不了文件、只能在对话里贴代码(无效交付)**。例:codex → [\"exec\",\"--sandbox\",\"workspace-write\",\"--skip-git-repo-check\",\"{{objective}}\"](workspace-write=可写工作目录;需要完整系统访问改 danger-full-access);claude → [\"-p\",\"{{objective}}\",\"--permission-mode\",\"bypassPermissions\",\"--output-format\",\"stream-json\",\"--verbose\"](stream-json 才能流式看到中间工具调用/过程,灵枢自动解析其 NDJSON 事件;text 则只出最终结果、无中间过程)。其它 CLI agent 按其自身的『跳过审批 / 允许写入』参数填。",
