...
extension LingShuState {
    ...
    private func registerAgentTool() -> LingShuAgentTool {
        let handler: @Sendable (String) async -> String = { [weak self] argumentsJSON in
            ...
            return await self.executeRegisterAgent(argumentsJSON: argumentsJSON)
        }
    ...
    private func executeRegisterAgent(argumentsJSON: String) async -> String {
        guard let data = argumentsJSON.data(using: .utf8),
              let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let id = (object["id"] as? String)?.trimmingCharacters(in: .whitespaces), !id.isEmpty,
              let name = object["name"] as? String, !name.isEmpty,
              let executable = object["executable"] as? String, !executable.isEmpty,
              let arguments = (object["args"] as? [Any])?.compactMap({ $0 as? String }), !arguments.isEmpty
        else { return "参数不全(要 id/name/executable/args)。" }

        let role = LingShuAgentPlugin.Role(rawValue: (object["role"] as? String) ?? "general") ?? .general
        let aliases = (object["aliases"] as? [Any])?.compactMap { $0 as? String } ?? []
        let capabilities = Self.agentCapabilitySpec(from: object["discover"])
        ...
        guard plugin.executableExists else {
            return "没注册:找不到可执行文件「\(executable)」。确认路径对、文件可执行。"
        }

        // 注册时真探活;探活失败仍登记但标不可用,避免到派活时才暴露。
        let probe = await LingShuAgentPluginStore.probeAvailability(plugin, workingDirectory: agentWorkingDirectory)
        ...
        _ = LingShuAgentPluginStore.register(registered)
        ...
        let discoveryText = capabilities != nil ? "(已配子技能发现,正在扫描其技能库,稍后即在「外部 agent 技能」单列)" : ""
        let statusText = probe.ok ? "探活通过·可用\(discoveryText)" : "探活发现不可用:\(probe.reason)"
        appendTrace(
            kind: probe.ok ? .system : .warning,
            actor: "agent插件",
            title: "注册 agent(\(probe.ok ? "可用" : "不可用"))",
            detail: "「\(name)」(\(role.rawValue))已入库,\(statusText)。"
        )
        if probe.ok {
            return "已把「\(name)」注册成 agent 插件(role=\(role.rawValue),**探活通过·当前可用**)\(discoveryText)。可 `@\(name) 目标` 声明式调用,或 run_agent 委托\(capabilities != nil ? ";它的子技能会单列进「外部 agent 技能」,可 @\(name)·某技能 精确调" : "")。"
        }
        return "「\(name)」已登记,但**探活发现当前不可用:\(probe.reason)**——已标记为不可用,不会被 @/派活(避免用时才暴露)。请先恢复(如该 CLI 登录 / 补凭据)后,重新 `register_agent` 探活即可启用。"
    }

    private static func agentCapabilitySpec(from rawValue: Any?) -> AgentCapabilitySpec? {
        guard let discovery = rawValue as? [String: Any] else { return nil }
        let format = ((discovery["format"] as? String)?.trimmingCharacters(in: .whitespaces))
            .flatMap { $0.isEmpty ? nil : $0 } ?? "skill-md"
        let directory = (discovery["skillsDir"] as? String)?.trimmingCharacters(in: .whitespaces)
        let registry = (discovery["registryFile"] as? String)?.trimmingCharacters(in: .whitespaces)
        let arguments = (discovery["args"] as? [Any])?.compactMap { $0 as? String }
        let hasRegistry = registry?.isEmpty == false
        let hasDirectory = directory?.isEmpty == false
        let hasArguments = arguments?.isEmpty == false
        guard hasRegistry || hasDirectory || hasArguments else { return nil }
        return AgentCapabilitySpec(
            discover: .init(
                args: hasArguments ? arguments : nil,
                skillsDir: hasDirectory ? directory : nil,
                registryFile: hasRegistry ? registry : nil,
                format: format
            ),
            enable: nil,
            install: nil
