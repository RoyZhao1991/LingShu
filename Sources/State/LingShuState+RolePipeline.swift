...
extension LingShuState {
    ...
    func planRolePipeline(task: String, agents: [(id: String, name: String)]) async -> [LingShuRoleStep] {
        ...
        - **【最高优先级】先尊重用户的明确指派**:用户在请求里 @ 了某 agent 并说了它干什么——「用 @X 做 / 开发 / 写」→ 那个 agent 当**工程执行**;「@Y 验收 / 把关 / 审 / 复核」→ 那个 agent 当**评审官**。**必须按用户指定把那个 agent 分到那个角色,绝不擅自换成灵枢或别人。** 用户没指派的角色(如架构师)你再决定谁来干。
