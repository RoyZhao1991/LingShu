...
extension LingShuState {
    ...
    /// 步骤1b·解析本任务 maker 该用的引擎:灵枢自己做时始终使用当前启用主脑。
    /// maker 的外部委托仍由后续 agent/plugin 机制完成。
    func resolveMakerEngine(taskRecordID: String?) -> LingShuMakerEnginePlan {
        _ = taskRecordID
        let providerLabel = modelProvider
        let engine = LingShuAgentEngineDescriptor(
        ...
        return .init(engine: engine, localAdapter: makeAgentModelAdapter())
    }
