...
struct LingShuExternalSensoryView: View {
    ...
    private var masterToggle: some View {
        HStack(spacing: 10) {
            Image(systemName: hub.masterEnabled ? "dot.radiowaves.left.and.right" : "antenna.radiowaves.left.and.right.slash")
                .foregroundStyle(hub.masterEnabled ? Color.lingHolo : .white.opacity(0.4))
        ...
        return VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 10) {
                Image(systemName: descriptor.channel.icon)
                    .foregroundStyle(status.isActive ? Color.lingHolo : .white.opacity(0.4))
                    .frame(width: 22)
                VStack(alignment: .leading, spacing: 2) {
    ...
    private var todoList: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(state.loc("关键待办", "Key to-dos"))
                .font(.system(size: 12, weight: .bold)).foregroundStyle(.white.opacity(0.75))
            ForEach(hub.phoneTodos) { todo in
                VStack(alignment: .leading, spacing: 3) {
                    HStack(spacing: 6) {
                        Image(systemName: "checkmark.circle").foregroundStyle(Color.lingHolo).font(.system(size: 11))
                        Text(todo.title).font(.system(size: 12, weight: .semibold)).foregroundStyle(.white.opacity(0.9))
                        Spacer()
                        if let due = todo.due {
                    ...
                    if !todo.actionSuggestion.isEmpty {
                        Text(todo.actionSuggestion).font(.system(size: 10.5, weight: .medium)).foregroundStyle(.white.opacity(0.5))
                    }
                    Text("\(todo.sourceApp)").font(.system(size: 9.5, weight: .medium)).foregroundStyle(.white.opacity(0.35))
                }
            ...
            if !hub.lastNote.isEmpty {
                Text(hub.lastNote).font(.system(size: 9.5, weight: .medium)).foregroundStyle(.white.opacity(0.3))
            }
        }
    }
    ...
    private var privacyNote: some View {
        HStack(alignment: .top, spacing: 8) {
            Image(systemName: "lock.shield").foregroundStyle(.white.opacity(0.4)).font(.system(size: 11))
            Text(state.loc(
    ...
    private func statusDot(_ status: LingShuExternalSensoryStatus) -> some View {
        ...
        case .disabled: .white.opacity(0.25)
        }
