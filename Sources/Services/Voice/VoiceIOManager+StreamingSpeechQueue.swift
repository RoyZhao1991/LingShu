        ...
        streamingSpeechDrainTask = Task { @MainActor [weak self] in
            while let self, !Task.isCancelled, self.speechGeneration == gen {
                ...
                guard let located = LingShuStreamingWAVHeader.locate(in: bytes), located.pcmStart < bytes.count else {
                    lingShuControlLog("TTS 无缝流式 seg#\(index) 跳过:WAV 头无效")
                    continue
                }
                // 采样率一致性:首段定基准,后续段不符则跳过(否则按首段固定 format 播 = 音高错/噪音)。
                if self.streamingSpeechFirstSampleRate == 0 {
                    self.streamingSpeechFirstSampleRate = located.sampleRate
                } else if abs(located.sampleRate - self.streamingSpeechFirstSampleRate) > 1 {
                    lingShuControlLog("TTS 无缝流式 seg#\(index) 采样率不符(\(located.sampleRate) vs \(self.streamingSpeechFirstSampleRate))跳过")
                    continue
                }
                ...
                guard pcm.count >= 2 else { continue }

                if self.streamingSpeechPlayer == nil {
                    guard let player = LingShuStreamingPCMPlayer(sampleRate: located.sampleRate, onOutputLevel: { [weak self] level in
                ...
                self.streamingSpeechPlayer?.enqueue(pcm16: pcm)
            }
