<div align="center">
  ...
  <p>
    ...
    ·
    <a href="https://github.com/RoyZhao1991/LingShu/releases/download/windows-v0.1.0-preview.25/Nous-Windows-x64-Setup.exe"><strong>Windows x64 技术预览</strong></a>
...
1. 下载 [Nous-Windows-x64-Setup.exe](https://github.com/RoyZhao1991/LingShu/releases/download/windows-v0.1.0-preview.25/Nous-Windows-x64-Setup.exe) 与 [SHA256SUMS.txt](https://github.com/RoyZhao1991/LingShu/releases/download/windows-v0.1.0-preview.25/SHA256SUMS.txt)。
...
Windows 技术预览直接运行与 macOS 完全相同的 Rust `RuntimeKernel`：单主任务串行、隔离子线程并行、可见的推理摘要与工具事件、人机阻断续跑、独立 checker、产物登记和内置预览。对话消息支持复制，用户消息可连同已记录附件一起回填输入框，编辑后再明确发送；这两种操作都不会自动发送，也不会改动原有历史。DesignKB 已作为模型可调用的演示文稿工具随安装包提供，本地插件包则通过共享内核注册，规范见[插件 SDK](./Docs/PLUGIN_SDK.md)。同一共享核心还可原样登记 Codex、Claude Code 与 Open Agent Skills 的标准 `SKILL.md` 目录：启动时只暴露有长度上限的目录，模型匹配任务后再按需读取完整说明与资源；导入不会自动执行随附脚本。当前只明确不包含 Windows 电脑直接控制与实时音视频感知。Windows 预览安装包尚未使用 Authenticode 签名，Windows 可能显示 SmartScreen 提示；安装前请核对 Release 中公布的 SHA-256。完整边界见 [Windows 版说明](./Docs/WINDOWS.md)。
