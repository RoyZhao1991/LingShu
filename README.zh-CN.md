<div align="center">
  ...
  <p>
    ...
    ·
    <a href="https://github.com/RoyZhao1991/LingShu/releases/download/windows-v0.1.0-preview.27/Nous-Windows-x64-Setup.exe"><strong>Windows x64 技术预览</strong></a>
...
1. 从 [v0.1.0-alpha.9 Release](https://github.com/RoyZhao1991/LingShu/releases/tag/v0.1.0-alpha.9) 下载 DMG 与对应 `.sha256` 文件。
2. 在终端校验下载文件：
...
1. 下载 [Nous-Windows-x64-Setup.exe](https://github.com/RoyZhao1991/LingShu/releases/download/windows-v0.1.0-preview.27/Nous-Windows-x64-Setup.exe) 与 [SHA256SUMS.txt](https://github.com/RoyZhao1991/LingShu/releases/download/windows-v0.1.0-preview.27/SHA256SUMS.txt)。
...
Windows 技术预览直接运行与 macOS 完全相同的 Rust `RuntimeKernel`：单主任务串行、隔离子线程并行、可见的推理摘要与工具事件、人机阻断续跑、独立 checker、产物登记、内置预览和自动长期记忆召回。新的“记忆”页面直接管理这套共享核心记忆，可搜索、筛选、新建、编辑和删除，并为敏感记录提供默认遮罩与逐条显式显示。对话消息支持复制，用户消息可连同已记录附件一起回填输入框，编辑后再明确发送；这两种操作都不会自动发送，也不会改动原有历史。DesignKB 已作为模型可调用的演示文稿工具随安装包提供，本地插件包则通过共享内核注册，规范见[插件 SDK](./Docs/PLUGIN_SDK.md)。同一共享核心还可原样登记 Codex、Claude Code 与 Open Agent Skills 的标准 `SKILL.md` 目录：启动时只暴露有长度上限的目录，模型匹配任务后再按需读取完整说明与资源；导入不会自动执行随附脚本。0.1.0-27 加入灵枢品牌安装界面，并修复旧发行人注册表布局升级失败的问题；升级与卸载不会删除对话和长期记忆目录。安装程序与应用元数据署名为 Roy Zhao，但安装包尚未使用 Authenticode 数字签名，Windows 仍可能显示“未知发布者”或 SmartScreen 提示；安装前请核对 Release 中公布的 SHA-256。当前明确不包含 Windows 电脑直接控制与实时音视频感知。完整边界见 [Windows 版说明](./Docs/WINDOWS.md)。
...
| 官网签名与公证发布 | [v0.1.0-alpha.9 已发布](https://github.com/RoyZhao1991/LingShu/releases/tag/v0.1.0-alpha.9)；内嵌 Loop Runtime 与公证 DMG 全新用户验证通过 |
