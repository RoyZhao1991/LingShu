...
## ✅ 更新(2026-06-15):真因锁定 = 公证(notarization),不只是签名
上机迭代后定位到**最后一跳是公证**,纠正之前"缺 Developer ID 证书"的判断。链路:
- 证书 `Developer ID Application: Yang Zhao (KM7N84AC9Y)` 已导入钥匙串、私钥在本机、`find-identity` 有效。
- 修了**两个真 bug**:① `codesign --deep` **不遍历 `Contents/Resources/*.driver`**→驱动一直 ad-hoc(`build-app.sh` 现单独签驱动再签 app);② `LingShuAudioDriverInstaller.installIfNeeded` 见已装就 `.alreadyInstalled` 跳过→本机残留的旧签名驱动永不被升级(现比对主执行文件字节,签名变就重装),`startMeetingConversation` 不再用 `isInstalled()` 短路。
- 自安装实测跑通:驱动落 `/Library/Audio/Plug-Ins/HAL/`、`codesign --verify --strict` 通过、`Authority=Developer ID Application / TeamID KM7N84AC9Y`、coreaudiod 重启。
- **但设备仍不出现。** `spctl -a -t install`:我们 = `Unnotarized Developer ID → rejected`;BlackHole = `Notarized Developer ID → accepted`。其余全同(MH_BUNDLE 双架构、工厂符号 `_LingShuAudioDriver_Create` 导出、`CFPlugInTypes`=HAL 类型 UUID、root:wheel、hardened runtime)。
- **结论:coreaudiod 静默只加载已公证的 HAL 插件。Developer ID 签名是必要非充分。**

### 打通方法(需你提供的新"组件"=公证凭据)
1. 一次性存 notarytool 凭据(任选其一):
   - `xcrun notarytool store-credentials <profile> --apple-id <你的AppleID> --team-id KM7N84AC9Y --password <app专用密码>`(app 专用密码在 appleid.apple.com 生成),或用 App Store Connect API key。
2. 一条命令打通(`build-app.sh` 已内置先签驱动→`notarytool submit --wait`+`stapler staple`→最后签 app):
   ```
   LINGSHU_SIGN_IDENTITY="Developer ID Application: Yang Zhao (KM7N84AC9Y)" \
   LINGSHU_NOTARY_PROFILE=<profile> bash Scripts/build-app.sh debug
   ```
   (或传 `LINGSHU_APPLE_ID` + `LINGSHU_APPLE_TEAM_ID` + `LINGSHU_APPLE_APP_PW`。)
3. 退旧实例 → `open dist/灵枢.app` → `meeting_converse_start` 触发自安装(输一次管理员密码)→ 『音频 MIDI 设置』出现"灵枢虚拟麦克风" → 会议里对一句话验证"对方听见灵枢"。
