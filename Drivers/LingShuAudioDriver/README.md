...
## ✅ 更新(2026-06-15):证书到位 + 签名 bug 已修
- `Developer ID Application: Yang Zhao (KM7N84AC9Y)` 已导入钥匙串、私钥在本机、`security find-identity -v -p codesigning` 列为有效身份。
- **修了 `build-app.sh` 的签名 bug**:`codesign --deep` **不遍历 `Contents/Resources/*.driver`**(只签 Frameworks/PlugIns 等常规嵌套),所以**驱动一直被漏签成 ad-hoc**——这才是 coreaudiod 静默拒载的真因(不只是"缺 Developer ID 证书")。脚本现在**单独对 `LingShuAudioDriver.driver` 用同一身份签**(`--options runtime` + 时戳,不套 app 的相机/麦克风 entitlements)再签 app。
- 实测重打包后随包驱动:`codesign -dv` → `Authority=Developer ID Application … / TeamID KM7N84AC9Y`;`codesign --verify --strict` → "satisfies its Designated Requirement"。
- **剩最后一跳(需在场):** 退旧实例 → `open dist/灵枢.app` → `meeting_converse_start` 触发自安装(输一次管理员密码)→ 『音频 MIDI 设置』确认"灵枢虚拟麦克风"出现 → 会议里对一句话验证"对方听见灵枢"。
