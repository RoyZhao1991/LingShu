...
/// LINGSHU MODIFICATION: Run the complete Grok agent runtime against in-memory channels.
///
/// This is the source-embedding entrypoint used by LingShu. It deliberately
/// keeps the ACP session protocol as the stable boundary while removing the
/// CLI process and stdin/stdout transport: the agent, its session registry,
/// planner, verifier, subagents and tools all live in the host process for the
/// lifetime of the application.
pub async fn run_embedded_agent(
    agent_config: &AgentConfig,
    mut incoming_messages: mpsc::UnboundedReceiver<String>,
    outgoing_messages: mpsc::UnboundedSender<String>,
    ...
    let (mut client_writer, agent_reader) = tokio::io::duplex(MAX_BUFFER_SIZE);
    let (agent_writer, client_reader) = tokio::io::duplex(MAX_BUFFER_SIZE);
        ...
        loop {
            tokio::select! {
                ...
                message = incoming_messages.recv() => {
                    ...
                    if client_writer.write_all(message.as_bytes()).await.is_err() { break; }
                }
            }
        }
        ...
        loop {
            tokio::select! {
                ...
                line = lines.next_line() => match line {
                    Ok(Some(line)) => {
                        if outgoing_messages.send(line).is_err() { break; }
                    }
                    Ok(None) | Err(_) => break,
                }
            }
    ...
    let result = local_set
        .run_until(async move {
            ...
            auth_manager.start_proactive_refresh(agent_shutdown.clone());
            ...
            tokio::select! {
                result = handle_io => result.map_err(|error| anyhow::anyhow!(error.to_string())),
                _ = agent_shutdown.cancelled() => Ok(()),
            }
        })
        .await;
