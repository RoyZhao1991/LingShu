...
// LINGSHU MODIFICATION: provider-neutral startup is integrated into the embedded runtime distribution.
    ...
    if let Some(command) = args.command.take() {
        match command {
            ...
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = xai_grok_telemetry::otel_layer::otel_guard();
                let mut config = xai_grok_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                xai_grok_shell::provider_catalog::materialize_saved_providers(&mut config)
