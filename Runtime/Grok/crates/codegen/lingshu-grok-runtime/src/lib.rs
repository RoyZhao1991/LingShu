...
fn emit(callback: Option<EventCallback>, context: usize, value: serde_json::Value) {
    ...
    let Ok(text) = CString::new(value.to_string()) else { return };
...
pub unsafe extern "C" fn lingshu_grok_runtime_start(
    ...
    let mut slot = runtime_slot().lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    if slot.is_some() {
        return 0;
    }

    let config_text = unsafe { CStr::from_ptr(config_json) };
    let Ok(config_text) = config_text.to_str() else { return 2 };
    let Ok(config) = serde_json::from_str::<RuntimeStartConfig>(config_text) else { return 3 };
    unsafe { std::env::set_var("GROK_HOME", &config.grok_home) };
    for (key, value) in &config.environment {
        unsafe { std::env::set_var(key, value) };
    }
    let Ok(agent_config) = parse_config(&config) else { return 4 };
    ...
    let thread = std::thread::Builder::new()
        ...
        .spawn(move || {
                ...
                .build()
            {
                Ok(runtime) => runtime,
                Err(error) => {
                    emit(callback, callback_context, serde_json::json!({
                        "kind": "runtime",
                        "status": "failed",
                ...
                let output_task = tokio::spawn(async move {
                    while let Some(message) = output_rx.recv().await {
                        let value = serde_json::from_str(&message)
                            .unwrap_or_else(|_| serde_json::json!({ "kind": "raw", "payload": message }));
...
pub unsafe extern "C" fn lingshu_grok_runtime_send(message: *const c_char) -> c_int {
    ...
    let Ok(message) = message.to_str() else { return 2 };
    let slot = runtime_slot().lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    let Some(runtime) = slot.as_ref() else { return 3 };
    runtime.input.send(message.to_string()).map(|_| 0).unwrap_or(4)
}
...
pub extern "C" fn lingshu_grok_runtime_stop() {
    let mut slot = runtime_slot().lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    let Some(mut runtime) = slot.take() else { return };
