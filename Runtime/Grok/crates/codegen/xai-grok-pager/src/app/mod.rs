//! LINGSHU MODIFICATION: supports provider-neutral API-token setup in addition to xAI authorization.
...
pub async fn run(
    ...
    crate::provider_setup::ensure_provider_ready(&raw_config, refreshed_auth.is_some()).await?;
    // Provider setup needs the real terminal for its first-run panel and masked
    // token input. Native stderr can be redirected after that interaction.
    ...
    let mut raw_config = xai_grok_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    xai_grok_shell::provider_catalog::materialize_saved_providers(&mut raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to apply provider configuration: {e}"))?;
